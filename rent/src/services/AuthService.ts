...
export async function getProprioById(id: string) {
    try {
        ...
        const users: (Proprio & { id: string })[] = querySnapshot.docs.map(
            (doc) => ({
                ...doc.data(),
                id: doc.id,
            }),
...
export async function apiSignIn(data: SignInCredential) {
    try {
        ...
        statsig
            ? await statsig?.updateUserAsync({ userID: user.email || 'n/a' })
            : null
...
export async function apiSignUpInside(
