...
import {
    ...
    getRequestType,
} from '../request/entities/AuthRequest'

export type TFunction = (key: string) => string

export function buildRequestEmailDetailsSection(
    request: IRequest,
    t: TFunction,
): string {
  ...
  `
    ...
    const onBehalf = general.is_for_other
    ...
    const generalSection = card('Request Details', generalRows, onBehalf, {
        showMeta: true,
    })
    ...
    const mainSection = (() => {
        ...
        if (typeRequest === 'bill' && (request as any).bill) {
            ...
            return card(
                'Bill',
                rows,
                b.description ? descriptionBlock(b.description) : '',
            )
        }
        ...
        if (typeRequest === 'divers' && (request as any).divers) {
            ...
            return card(
                'Divers',
                rows,
                d.description ? descriptionBlock(d.description) : '',
            )
        }
        ...
        if (typeRequest === 'capex' && (request as any).capex) {
            ...
            return card(
                'Capex',
                rows,
                c.decripstion ? descriptionBlock(c.decripstion) : '',
            )
        }
        ...
        if (typeRequest === 'telecom' && (request as any).telecom) {
            ...
            const categorieLabel = getRequestCategorieById(
                t,
                type,
                tel.categorie,
            )
            ...
            const desc = tel.description
                ? descriptionBlock(tel.description)
                : ''
        ...
        return card(
            ...
            [
                row('Type', typeRequest || '-'),
                row('Beneficiary', general.beneficiaryName ?? '-'),
            ].join(''),
