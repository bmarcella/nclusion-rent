...
export const STATUS_MAP = {
    new: {
        label: 'New',
        subject: 'Nouvelle Requête',
        template: NewReqEmailTemplate,
    },
    approved: {
        label: 'Approved',
        subject: 'Requête Approuvée',
        template: ReqApprovedEmailTemplate,
    },
    paid: {
        label: 'Paid',
        subject: 'Requête Payée',
        template: ReqPaidEmailTemplate,
    },
    reject: {
        label: 'Rejected',
        subject: 'Requête Rejettée',
        template: ReqRejectedEmailTemplate,
    },
    canceled: {
        label: 'Canceled',
        subject: 'Requête Annullée',
        template: ReqStatusChangeEmailTemplate,
    },
    reminder: {
        label: 'Canceled',
        subject: 'Requête - Nouvelle status ',
        template: ReqStatusChangeEmailTemplate,
    },
    ac: {
        label: 'Canceled',
        subject: 'Requête - Nouvelle status ',
        template: ReqStatusMyChangeEmailTemplate,
    },
...
const ProcessNewMail = forwardRef<HTMLDivElement, any>(({}, ref) => {
    ...
    const fetchRule = async (
        ...
        try {
            ...
            if (request?.status == 'approved') {
                landlords_2 = await fetchProprio(nsregions, [
                    'assist_accountant',
                    'accountant',
                ])
                landlords_3 = await fetchProprioByRole([
                    'super_accountant',
                    'super_manager',
                ])
        ...
        if (proprios.length > 0) {
            try {
                if (previousStatus) {
                    ...
                    console.log(
                        'APPROVED SENT TO ACCOUNTANT and MANAGER',
                        type,
                        res,
                    )
                }
    ...
    const sendMailNotificationToApi = async (
        payload: MailData,
        previousStatus?: string,
    ) => {
        const reqText = payload.action?.request || ''
        ...
        if (request?.status == 'completed') {
            try {
                if (previousStatus) {
                    const creator = await getProprioById(
                        payload.request.createdBy,
                    )
