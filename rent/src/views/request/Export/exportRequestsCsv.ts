...
export function exportRequestsCsv(params: {
  ...
  const { requests, t, filename = `requests-${Date.now()}.csv`, proprios } = params;
  const getProprio = (created_by: string) => {
    return proprios.get(created_by);
  }
  const filteredRequests = requests.filter(r => r.general?.id_region_user != 10);
  ...
  const rows = filteredRequests.map((r: IRequest) => {
    ...
    const regionLabel =
      g.id_region_user != null ? (getRegionsById(g.id_region_user)?.label ?? String(g.id_region_user)) : "-";

    const categorieLabel =
      categorie != null ? getRequestCategorieById(t, r.requestType, categorie) : "-";
    ...
    return {
      ...
      "Creator": name,
      "Creator role": role,
      "Approved by Coordonator" : getProprio(r.regionalApproved_by)?.fullName ?? r.regionalApproved_by ?? "-",
      "Approved by accountant" : getProprio(r.accountantApproval)?.fullName ?? r.accountantApproval ?? "-",
      "Approved by manager" : getProprio(r.managerGlobalApproval)?.fullName ?? r.managerGlobalApproval ?? "-",
      "Rejected by" : getProprio(r.rejectedBy ?? "")?.fullName ?? r.rejectedBy ?? "-",
      "Cancelled by" : getProprio(r.cancelledBy ?? "")?.fullName ?? r.cancelledBy ?? "-",
      "Completed by" : getProprio(r?.completedBy ?? "")?.fullName ?? r.completedBy ?? "-",
    };
  });
