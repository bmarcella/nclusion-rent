...
export default function FilterAuthRequest({
    ...
    const reqTypeOpts = useMemo(
        () => toOptions(reqTypeOptions),
        [reqTypeOptions],
    )
    ...
    return (
        <div className="grid grid-cols-1 md:grid-cols-6 gap-4 bg-white p-4 rounded">
            ...
            <div className="col-span-2">
                ...
                <Input
                    placeholder={
                        t('filters.userPlaceholder') || 'user id or email'
                    }
                    value={isUser ?? ''}
            ...
            <div className="col-span-2">
                <label className="block text-sm mb-1">
                    {t('filters.roles') || 'Roles'}
                </label>
                <Select<Option, true>
                    isMulti
                    placeholder={
                        t('filters.rolesPlaceholder') || 'Select roles'
                    }
                    ...
                    onChange={(vals: Option[]) =>
                        setRoles(vals?.map((v) => String(v.value)) ?? [])
                    }
                />
                <div className="text-xs text-gray-500 mt-1">
                    {t('filters.rolesNote') ||
                        'Up to 10 values (Firestore array-contains-any limit).'}
                </div>
            </div>
            ...
            <div className="col-span-2">
                <label className="block text-sm mb-1">
                    {t('filters.reqTypes') || 'Req Types'}
                </label>
                <Select<Option, true>
                    isMulti
                    placeholder={
                        t('filters.reqTypesPlaceholder') || 'Select types'
                    }
                    ...
                    onChange={(vals: Option[]) =>
                        setReqType(vals?.map((v) => Number(v.value)) ?? [])
                    }
                />
            </div>
            ...
            <div className="col-span-2">
                <label className="block text-sm mb-1">
                    {t('filters.region') || 'Region'}
                </label>
                <Select<Option, false>
                    isClearable
                    placeholder={
                        t('filters.regionPlaceholder') || 'Select region'
                    }
            ...
            <div className="col-span-2">
                ...
                <Select<Option, false>
                        ...
                        [
                            {
                                value: 'created-desc',
                                label:
                                    t('filters.createdDesc') ||
                                    'Created (newest first)',
                            },
                            {
                                value: 'created-asc',
                                label:
                                    t('filters.createdAsc') ||
                    ...
                    onChange={(opt: Option | null) =>
                        setSort((opt?.value as SortKey) ?? 'created-desc')
                    }
                />
            </div>
