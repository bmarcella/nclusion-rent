...
export const ViewAuthRequest: React.FC = () => {
    ...
    const constraints = useMemo<QueryConstraint[]>(() => {
        ...
        if (filter.roles?.length) {
            cs.push(
                where('roles', 'array-contains-any', filter.roles.slice(0, 10)),
            )
        }
    ...
    const setCurrency = async (
        ...
        try {
            ...
            setMessage(
                'Erreur lors de la modificationde la devise de la requete',
            )
            setAlert('danger')
        }
    }
            ...
            <div className="w-full mt-6 bg-gray-50 dark:bg-gray-700 rounded-sm p-6 shadow">
                        ...
                        {!loading &&
                            rows.map((r, i) => (
                                <Tr key={r.id}>
                                    ...
                                    <Td>
                                        {Array.isArray(r.reqType)
                                            ? r.reqType.join(', ')
                                            : '-'}
                                    </Td>
                                    <Td>
                                        {r.region_id
                                            ? getRegionsById(r.region_id)
                                                  .capital
                                            : '-'}
                                    </Td>
                                    <Td>
                ...
                <div className="flex items-center justify-between mt-4">
                    <Pagination
                        ...
                        onChange={(next) => {
                            if (next === page + 1)
                                return hasNext && fetchPage(next)
                            if (next === page - 1)
                                return page > 1 && fetchPage(next)
                            fetchPage(next)
                        }}
                    />
