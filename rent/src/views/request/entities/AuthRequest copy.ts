...
const StatusTrain = (t: any) => {
   return {
     value: 'preApproval', 
     label: t('request.status.preApproval') ,
     next : {
       value: 'regionalApproval', 
       label: t('request.status.regionalApproval') ,
       next :  { 
         value: 'accountantRegionalApproval', 
         label: t('request.status.accountantRegionalApproval'),
         next :  { 
             value: 'managerGlobalApproval',
             label: t('request.status.managerGlobalApproval'),
             next:  { 
               value: 'approved', 
               label: t('request.status.approved'),
               next: { 
                   value: 'completed',
                   label: t('request.status.completed') 
               },
            },
        },
       },
