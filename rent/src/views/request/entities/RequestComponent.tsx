...
import ImageLordComp, {
    LordImage,
} from '@/views/bank/show/components/ImageLord'
        ...
        <>
            <Section title="Géneral">
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                    {regions.length > 1 && regions.length != 0 && (
                        <Field
                            label="Région"
                            errors={{
                                key: 'id_region_user',
                    ...
                    {regions.length == 1 && (
                        <Field label="Region">
                            <Input disabled value={regions[0].capital}></Input>
                        </Field>
                    )}
        ...
        <Section title="Bill & Autres">
            ...
            <Field label="Amount">
                <Input
                    type="number"
                    step="0.01"
                    {...register('bill.price', { valueAsNumber: true })}
                />
            </Field>
            <Field label="Description">
                <Input textArea {...register('bill.description')} />
            </Field>
            <Field label="Target date">
                <Input
                    type="date"
                    {...register('bill.target_date', { valueAsDate: true })}
                />
            </Field>
        </Section>
    )
        ...
        <Section title="Divers">
            ...
            <Field label="Amount">
                <Input
                    type="number"
                    step="0.01"
                    {...register('divers.price', { valueAsNumber: true })}
                />
            </Field>
            <Field label="Description">
                <Input textArea {...register('divers.description')} />
            </Field>
            <Field label="Target date">
                <Input
                    type="date"
                    {...register('divers.target_date', { valueAsDate: true })}
                />
            </Field>
        </Section>
    )
    ...
    return (
        <Section title="Capex">
            ...
            <Field label="Quantity">
                <Input
                    type="number"
                    {...register('capex.quantity', { valueAsNumber: true })}
                />
            </Field>
            <Field label="Prix unitiare">
                <Input
                    type="number"
                    step="0.01"
                    {...register('capex.unit_price', { valueAsNumber: true })}
                />
            </Field>
            <Field label="Prix Total">
                <Input
                    ...
                    type="number"
                    step="0.01"
                    {...register('capex.price', { valueAsNumber: true })}
                />
            </Field>
            <Field label="Fournisseur">
                <Input {...register('capex.provider')} />
            </Field>
            <Field label="Target date">
                <Input
                    type="date"
                    {...register('capex.target_date', { valueAsDate: true })}
                />
            </Field>
            <Field label="Description">
        ...
        <Section title="Locomotif">
            ...
            <Field label="Type">
                <Controller
                    ...
                    render={({ field }) => (
                        <Select
                            options={LocomotifTypeEnum.options.map((p) => ({
                                label: p,
                                value: p,
                            }))}
                            onChange={(opt) =>
            ...
            <Field label="Plaque">
                <Input {...register('locomotif.plaque')} />
            </Field>
            <Field label="Founisseur/Vendeur">
                <Input {...register('locomotif.provider')} />
            </Field>
            <Field label="Price">
                <Input
                    type="number"
                    step="0.01"
                    {...register('locomotif.price', { valueAsNumber: true })}
                />
            </Field>
            <Field label="Description">
                <Input
                    placeholder="Text area example"
                    {...register('locomotif.description')}
                    textArea
                />
            </Field>
        </Section>
    ...
    return (
        <Section title="Information Bancaire">
            <Field label="Bank name">
                <Input {...register('BankInfo.BankName')} />
            </Field>
            <Field label="Account name">
                <Input {...register('BankInfo.AccountName')} />
            </Field>
    ...
    return (
        <Section title="Documents">
            <div className="space-y-3">
                ...
                <Button
                    type="button"
                    onClick={() => append({ type: undefined as any })}
                >
                    Add document
                </Button>
            </div>
...
export function TelecomFields({ t, categories }: any) {
    ...
    useEffect(() => {
        ...
        requestAnimationFrame(() => {
            cardRefs.current[lastId]?.scrollIntoView({
                behavior: 'smooth',
                block: 'start',
            })
        })
                    ...
                    >
                        ...
                        <div className="grid grid-cols-2 gap-3 mt-2">
                            <Input
                                ...
                                {...register(
                                    `telecom.plans.${idx}.beneficiary` as const,
                                )}
                            />
                            <Controller
                                control={control}
                                ...
                                render={({ field }) => (
                                    <Select
                                        options={ProviderTelecomEnum.options.map(
                                            (p) => ({ label: p, value: p }),
                                        )}
                                        onChange={(opt) =>
                                            field.onChange(String(opt?.value))
                            ...
                            <Input
                                ...
                                {...register(
                                    `telecom.plans.${idx}.plan_type` as const,
                                )}
                            />
                            <Input
                                type="date"
                                {...register(
                                    `telecom.plans.${idx}.start_date` as const,
                                    { valueAsDate: true },
                                )}
                            />
                            <Input
                                type="date"
                                {...register(
                                    `telecom.plans.${idx}.end_date` as const,
                                    { valueAsDate: true },
                                )}
                            />
                            <Input
                                ...
                                placeholder="Price"
                                {...register(
                                    `telecom.plans.${idx}.price` as const,
                                    { valueAsNumber: true },
                                )}
                            />
                            <Input
                                placeholder="NIF/NIN"
                                {...register(
                                    `telecom.plans.${idx}.id_card` as const,
                                )}
                            />
                            <Error
                                errors={{
    ...
    return (
        <Section title="Opex (Achat de Matériel / Fournitures)">
            ...
            <div className="space-y-3">
                ...
                {fields.map((f, idx) => (
                    ...
                    >
                        ...
                        <Input
                            ...
                            type="number"
                            placeholder="Qty"
                            {...register(`opex.items.${idx}.quantity`, {
                                valueAsNumber: true,
                            })}
                        />
                        ...
                        <Input
                            ...
                            {...register(`opex.items.${idx}.unit_price`, {
                                valueAsNumber: true,
                            })}
                        />
                        ...
                        <Input
                            ...
                            {...register(`opex.items.${idx}.total_price`, {
                                valueAsNumber: true,
                            })}
                        />
                ...
                <Button
                    type="button"
                    onClick={() => append({ quantity: 1, unit_price: 0 })}
                >
                    Add item
                </Button>
            </div>
...
export function TransportFields({ t, region, categories }: any) {
    ...
    const { fields, append, remove } = useFieldArray({
        name: 'transport_logistique.items',
        control,
    })
    ...
    useEffect(() => {
        const region =
            Regions.find(
                (option) => Number(option.value) === Number(sregion),
            ) || null
        if (region) {
    ...
    useEffect(() => {
        const region =
            Regions.find(
                (option) => Number(option.value) === Number(tsregion),
            ) || null
        if (region) {
...
export function LeasePaymentFields({ t, categories }: any) {
    ...
    const fetchReq = async (bankId: string) => {
        ...
        try {
            ...
            setLoading(true)
            const baseQuery =
                ExpenseRequestDoc as CollectionReference<DocumentData>
    ...
    useEffect(() => {
        ...
        if (can) {
            ...
            setValue(
                'lease_payment.rentCost',
                cbank.final_rentCost ?? cbank.rentCost,
            )
            setValue('lease_payment.yearNumber', cbank.yearCount as any)
            setValue(
                'lease_payment.whoApproveTheBank',
                cbank.finalDecision.createdBy,
            )
            setValue(
                'lease_payment.description',
                ` Location pour une nouvelle bank : ${cbank.bankName} 
    ...
    return (
                ...
                {!loading && reqs.length > 0 && (
                    <div className="min-h-[160px] flex items-center justify-center">
                        <h6 className="text-red-500">
                            {' '}
                            Cette bank a déja une requête en cours
                        </h6>
                    </div>
                )}
                {can && (
                    <div className="grid grid-cols-2 gap-3">
                        <Field label="Nom du proprietaire">
                            <Input
                                disabled={true}
                                {...register('lease_payment.landlordName')}
                            />
                        </Field>
                        ...
                        >
                            <Input
                                type="date"
                                {...register('lease_payment.start_date')}
                            />
                        </Field>
                        <Field
                            label="End date"
                        ...
                        >
                            <Input
                                type="date"
                                {...register('lease_payment.end_date')}
                            />
                        </Field>
                        ...
                        >
                            <Input
                                type="number"
                                {...register('lease_payment.yearNumber', {
                                    valueAsNumber: true,
                                })}
                            />
                        </Field>
                        <Field label="Rent cost">
                            <Input
                                disabled={true}
                                type="number"
                                step="1000"
                                {...register('lease_payment.rentCost', {
                                    valueAsNumber: true,
                                })}
                            />
                        </Field>
        ...
        <Field label="Created by"><Input disabled={true} {...register("lease_payment.create_by")} /></Field> */}
                ...
                {can && (
                    <>
                        ...
                        <Field label="Description">
                            <Input
                                textArea
                                {...register('lease_payment.description')}
                            />
                        </Field>
                    </>
                )}
            ...
            {cbank && (
                <CollapsSection title={'Detail Bank'} defaultOpen={false}>
                    <>
    ...
    useEffect(() => {
        const run = async () => {
            if (contrat && can) {
                console.log(contrat)
                const total =
                    Number(contrat.montant_total) + Number(contrat.transport)
                ...
                setValue(
                    'bank_renovation.vendor_name',
                    contrat.landlord?.fullName,
                )
    ...
    const fetchTasks = async (region: number, step: string) => {
        try {
            ...
            q = query(
                ...
                where('regionsId', 'array-contains', region),
                orderBy('createdAt', 'desc'),
            )
            const snapshot = await getDocs(q)
        ...
        <>
            <Section title="Bank Renovation">
                ...
                {contrats && contrats.length > 0 && (
                    <Field label="Contrat de renovation">
                        <Controller
                            control={control}
                ...
                {!loading && contrats && contrats.length == 0 && (
                    <div className="min-h-[50px] flex items-center justify-center">
                        <b className="text-red-400 text-center">
                            0 contrat de renovation trouvé
                        </b>
                    </div>
                )}
                {!loading && reqs.length > 0 && (
                    <div className="min-h-[160px] flex items-center justify-center">
                        <h6 className="text-red-500">
                            {' '}
                            Cet contrat a déja une requête en cours
                        </h6>
                    </div>
                )}
                {can && (
                    <>
                        <div className="grid grid-cols-2 gap-3">
                            <Field label="Start date">
                                <Input
                                    type="date"
                                    {...register('bank_renovation.start_date', {
                                        valueAsDate: true,
                                    })}
                                />
                            </Field>
                            <Field label="End date">
                                <Input
                                    type="date"
                                    {...register('bank_renovation.end_date', {
                                        valueAsDate: true,
                                    })}
                                />
                            </Field>
                        ...
                        <div className="grid grid-cols-2 gap-3">
                            <Field label="Founisseur de service">
                                <Input
                                    readOnly
                                    {...register('bank_renovation.vendor_name')}
                                />
                            </Field>
                        ...
                        <Field label="Description">
                            <Input
                                textArea
                                {...register('bank_renovation.description')}
                            />
                        </Field>
                    </>
                )}
            ...
            {contrat && (
                <CollapsSection title={'Details sur le contrat de Revovation'}>
                    <ContractDeService
                        contract={contrat}
                        employee={contrat.landlord}
                        proprio={contrat.employee}
                    ></ContractDeService>
                    <></>
                </CollapsSection>
            )}
