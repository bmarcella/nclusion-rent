...
export const ProviderTelecomEnum = z.enum([
    'Natcom',
    'Digicel',
    'Access Haiti',
    'Starlink',
...
export const CapexSchema = z.object({
    ...
    unit_price: z.coerce
        .number()
        .gt(0, { message: 'Unit Price must be greater than 0' }),
...
export const TelecomPlanSchema = z
    .object({
        beneficiary: z.string().min(1),
...
export const TransportSchema = z
    .object({
        ...
        items: z
            ...
            .min(1, 'Vous devez ajouter au moins un materiel'),
    })
    .superRefine((val, ctx) => {
...
export const LeasePaymentSchema = z
    .object({
        type: z.string().optional(),
        categorie: z.coerce.number().nonnegative(),
            ...
            case 'divers':
                if (!val.divers)
                    ctx.addIssue({
                        code: z.ZodIssueCode.custom,
                        path: ['divers'],
                        message: 'Divers section is required',
                    })
                break
            case 'legal':
                if (!val.legal)
                    ctx.addIssue({
                        code: z.ZodIssueCode.custom,
                        path: ['legal'],
                        message: 'Legal section is required',
                    })
                break
            case 'bill':
                if (!val.bill)
                    ctx.addIssue({
                        code: z.ZodIssueCode.custom,
                        path: ['bill'],
                        message: 'Bill section is required',
                    })
                break
            case 'capex':
                if (!val.capex)
                    ctx.addIssue({
                        code: z.ZodIssueCode.custom,
                        path: ['capex'],
                        message: 'Capex section is required',
                    })
                break
            case 'locomotif':
                if (!val.locomotif)
                    ctx.addIssue({
                        code: z.ZodIssueCode.custom,
                        path: ['locomotif'],
                        message: 'Locomotif section is required',
                    })
                break
            case 'telecom':
                if (!val.telecom)
                    ctx.addIssue({
                        code: z.ZodIssueCode.custom,
                        path: ['telecom'],
                        message: 'Telecom section is required',
                    })
                break
            case 'opex':
                if (!val.opex)
                    ctx.addIssue({
                        code: z.ZodIssueCode.custom,
                        path: ['opex'],
                        message: 'Opex section is required',
                    })
                break
            case 'transport_logistique':
                if (!val.transport_logistique)
                    ctx.addIssue({
                        code: z.ZodIssueCode.custom,
                        path: ['transport_logistique'],
                        message: 'Transport section is required',
                    })
                break
            case 'bank_renovation':
                if (!val.bank_renovation)
                    ctx.addIssue({
                        code: z.ZodIssueCode.custom,
                        path: ['bank_renovation'],
                        message: 'Bank renovation section is required',
                    })
                break
            case 'lease_payment':
                if (!val.lease_payment)
                    ctx.addIssue({
                        code: z.ZodIssueCode.custom,
                        path: ['lease_payment'],
                        message: 'Lease payment section is required',
                    })
                break
        }
