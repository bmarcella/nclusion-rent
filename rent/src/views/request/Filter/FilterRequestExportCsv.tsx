...
function FilterRequestCsv({ onChange, data }: Props) {
  ...
  const fetchProprio = async () => {
    try {
      ...
      q = query(baseQuery, orderBy('fullName'), where('type_person', 'in', ['assist_manager', 'manager', 'assist_coordonator', 'accountant', 'assist_accountant', 'super_accountant', 'super_manager', 'coordonator']));
