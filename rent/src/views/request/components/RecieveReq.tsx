...
export default function RecieveReq() {
    ...
    const fetchRule = async () => {
        ...
        const q =
            role == 'admin'
                ? query(AuthRequestDoc, where('roles', 'array-contains', role))
                : query(
                      AuthRequestDoc,
        ...
        const objs = snapshot.docs.map((docSnap) => {
    ...
    return (
        <>
            {nquery && rules && rules.length > 0 && (
                <ShowReq recieve={nquery} />
            )}
            ...
            {!loadingRules && rules.length == 0 && (
                <>
                    <div className="flex items-center justify-center text-center">
                        <h3>
                            {
                                "Vous n'avez pas encore reçu de requête pour' l'instant"
                            }{' '}
                        </h3>
                    </div>
                </>
