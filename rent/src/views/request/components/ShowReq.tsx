...
function ShowReq({
    ...
    const constraints = useMemo<AnyConstraint[]>(() => {
        ...
        const rg =
            !filter?.regions || filter?.regions == null || filter?.regions == 0
                ? reg
                : [filter.regions]
        // REGION
        if (role !== 'admin' && rg.length > 0) {
            if (rg.length == 1) {
                filters.push(where('general.id_region_user', '==', rg[0]))
            }

            if (rg.length > 1) {
                filters.push(where('general.id_region_user', 'in', rg))
            }
        }

        if (role === 'admin' && filter?.regions) {
            filters.push(
                where('general.id_region_user', 'in', [filter.regions]),
            )
        }
        ...
        if (filter?.amount?.min != null)
            filters.push(where('amount', '>=', filter.amount.min))
        if (filter?.amount?.max != null)
            filters.push(where('amount', '<=', filter.amount.max))
        if (filter?.amount?.currency != null)
            filters.push(
                where('general.currency', '==', filter.amount.currency),
            )

        // DATE
        if (filter?.date?.start)
            filters.push(where('createdAt', '>=', filter.date.start))
        if (filter?.date?.end)
            filters.push(where('createdAt', '<=', filter.date.end))
        ...
        const topLevelFilter = filters.length === 0 ? [] : [and(...filters)]
    ...
    const fetchPage = async (pageNumber: number) => {
        ...
        try {
            ...
            const items: IRequest[] = snap.docs.map((d) => ({
                id: d.id,
                ...(d.data() as any),
            }))
                        ...
                        <div className="font-medium" title={row.original.id}>
                            <Tag
                                className={classNames(
                                    getTypeRequestTagClasses(
                                        row.original.requestType,
                                    ),
                                    'mb-1 mr-1',
                                )}
                            >
                                {t('request.' + row.original.requestType)}
                            </Tag>
                        </div>
                        <div
                            className="font-medium"
                            title={getCategorieName(t, row.original)}
                        >
                            <Tag className="mt-1">
                                {' '}
                                {String(
                                    getCategorieName(t, row.original),
                                ).substring(0, 30)}{' '}
                            </Tag>
                        </div>
                    </div>
                    ...
                    <div className="min-w-[160px]">
                        <div className="font-medium">
                            ...
                            <Currency
                                ...
                                tag={row.original.general?.currency}
                            ></Currency>
                        </div>
                    </div>
                ),
                ...
                cell: ({ row }) => (
                    <div className="min-w-[160px]">
                        <div className="font-medium">
                            ...
                            {
                                getRegionsById(
                                    row.original.general!.id_region_user!,
                                ).label
                            }
                        </div>
                    </div>
                                    ...
                                    .map((a) => {
                                        ...
                                        return (
                                            <div key={k} className="mt-1">
                                                <Badge
                                                    content={`${a.status_from} <=> ${a.status_to}`}
                                                />
                                            </div>
                                        )
                                    })}
            ...
            },
            {
                ...
                cell: ({ row }) => (
                    <div className="min-w-[160px]">
                        ...
                        <div className="font-medium">
                            {' '}
                            pour <b>{row.original?.general?.beneficiaryName}</b>
                        </div>
                        {
                            <div className="font-medium">
                                {' '}
                                ...
                                <b>
                                    {row.original?.expiration_date
                                        ? formatRelative(
                                              row.original?.expiration_date.toDate?.() ||
                                                  row.original?.expiration_date,
                                              new Date(),
                                              { locale: fr },
                                          )
                                        : formatRelative(
                                              add7DaysToExpirationDate(
                                                  row.original?.expiration_date,
                                              ).toDate?.() ||
                                                  add7DaysToExpirationDate(
                                                      row.original
