    ...
    return (
        <>
            <Timeline className="mt-4">
                {data.historicApproval.length > 0 &&
                    data.historicApproval.map((step: any, index) => {
                        return (
                            <Timeline.Item
                                key={index}
                                className="mb-4"
                            ...
                            >
                                <p className="my-1 flex ">
                                    <span className="font-semibold text-gray-900 dark:text-gray-100">
                                        {step.by_who && (
                                            <UserName userId={step.by_who} />
                                        )}{' '}
                                        - ({String(step.by_who).substring(0, 4)}
                                        )
                                    </span>
                                    <span className="mx-2">
                                        {' '}
                                    ...
                                    <Badge
                                        className={getColor(step?.status_to)}
                                    >
