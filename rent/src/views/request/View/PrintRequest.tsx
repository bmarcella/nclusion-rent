...
const PrintRequest: React.FC<Props> = ({ request }) => {
            ...
            <Card className="p-4 space-y-2">
                ...
                <div className="grid grid-cols-3 md:grid-cols-3 gap-3 text-sm mt-4">
                    <div>
                        ...
                        <div>
                            {formatMoney(price)} {request?.general?.currency}
                        </div>
                    </div>
                    <div>
                    ...
                    <div>
                        ...
                        <div>
                            {getRequestCategorieById(t, type, categorie)}{' '}
                        </div>
                    </div>
                    <div>
                        ...
                        <div>
                            {d?.type
                                ? getRequestType(t, type, categorie, d?.type)
                                : '-'}{' '}
                        </div>
                    </div>
                </div>
        ...
        return (
            <Card className="p-4 space-y-2">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-3 text-sm mt-4">
                    <div>
                        <b>Amount:</b> {formatMoney(b.price)}{' '}
                        {request.general?.currency}
                    </div>
                    <div>
                        <b>Target date:</b> {formatDate(b.target_date)}
                    </div>
                    <div>
                        <div className="font-bold">Categorie</div>
                        <div>
                            {getRequestCategorieById(t, type, b.categorie)}{' '}
                        </div>
                    </div>
                    <div>
                        <div className="font-bold">Type</div>
                        <div>
                            {b?.type
                                ? getRequestType(t, type, b.categorie, b?.type)
                                : '-'}{' '}
                        </div>
                    </div>
                </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-3 text-sm">
                    ...
                    {ntype && (
                        <div>
                            <div className="font-bold">Type</div>
                            <div>
                                {getRequestType(t, type, categorie, ntype)}{' '}
                            </div>
                        </div>
                    )}
                    <div>
        ...
        return (
            ...
            <div>{getRequestType(t, type, categorie, request.opex?.type)} </div>
                    ...
                    <div>
                        <div className="font-bold">Amount</div>
                        <div>
                            {formatMoney(amount)} {request?.general?.currency}
                        </div>
                    </div>
            ...
            <Card className="p-4 space-y-3">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-3 text-sm">
                    ...
                    <div>
                        ...
                        <div>
                            {formatMoney(l.rentCost)} {general?.currency}
                        </div>
                    </div>
                    <div>
                        ...
                        <div>
                            {' '}
                            <UserName userId={l.whoApproveTheBank} />
                        </div>
                    </div>
                    <div>
    ...
    const renderBankInfo = () => {
        ...
        return (
            <Card className="p-4 space-y-2">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-3 text-sm">
                    <div>
                        <b>Bank name:</b> {b.BankName}
                    </div>
                    <div>
                        <b>Account name:</b> {b.AccountName}
                    </div>
                    <div>
                        <b>Account number:</b> {b.AccountNumber}
                    </div>
                    {b.SWIFT && (
                        <div>
                            <b>SWIFT:</b> {b.SWIFT}
            ...
            <Card className="p-4 space-y-3">
                ...
                <div className="grid grid-cols-1 md:grid-cols-4 gap-3 text-sm mt-4">
                    ...
                    <div>
                        <div className="font-bold">Amount</div>
                        <div>
                            {formatMoney(t.amount)} {request?.general?.currency}
                        </div>
                    </div>
                    <div>
                        <div className="font-bold">Categorie</div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-3 text-sm mt-4">
                    <div>
                        ...
                        <div>
                            Region: {getRegionsById(t.From?.region).label}
                        </div>
    ...
    const renderImage = () => {
        ...
        return (
            <CollapsSection title={'Documents'}>
                <ImageReq
                    reqId={request.id}
                    userId={userId || ''}
                    isEdit={true}
                    owner={request.createdBy == userId}
                    end={end}
                ></ImageReq>
            </CollapsSection>
        )
            ...
            <div ref={contentRef} className="space-y-4 p-4">
                ...
                <Card className="p-4 space-y-2">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-3 text-sm">
                        <div>
                            <b>Request Type:</b> {request.general?.type_request}
                        </div>
                        <div>
                            <b>Payment Method:</b>{' '}
                            {request.general?.paymentMethod}
                        </div>
                        <div>
                            <b>Currency:</b> {request.general?.currency}
                        </div>
                        <div>
                            <b>Beneficiary:</b>{' '}
                            {request.general?.beneficiaryName || '-'}
                        </div>
                        <div>
