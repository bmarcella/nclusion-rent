...
const Home = () => {
    ...
    useEffect(() => {
        const regs = async () => {
            ...
            const report = await fetchReportPerReport(ReportSteps as any, q)
            setRegions(regions)
            setValues(values)
            setReport(report)
        }
                    ...
                    >
                        ...
                        <div className="grid grid-flow-col auto-cols-max gap-4">
                            {report.map((item, index) => (
                                <div
                                    key={index}
                                    className="rounded bg-white m-1"
                                >
            ...
            {userId && is_ai && (
                <div className="mt-8 flex flex-col gap-2 items-center justify-center w-full">
                    <h1 className="text-4xl font-bold">
                        Bienvenue sur la plateforme de gestion immobilière
                    </h1>
                    <p className="mt-4 text-lg">
                        Utilisez le bouton en haut à droite pour ouvrir le menu.
                    </p>
                    <a
                        href="/proprio/add"
