...
export const ShowBankDetailsBase = () => {
    ...
    const onContratOk = async () => {
        ...
        const lease: BankLease = {
            ...
            date_fin: bank
                ? getEndDateYear(bank?.date, Number(bank.yearCount))
                : new Date(),
    ...
    const SaveBankTasks = async () => {
        const hasRenov =
            (bank?.renovationDetails?.neededSecurity?.length ?? 0) > 0 ||
    ...
    const SaveHistory = async (reject: any, dec?: HistoricDecision) => {
        if (bankId)
            await updateBankById(bankId, reject).then(async () => {
                if (dec) {
                    dec.bankId = bankId
                    console.log(dec)
                    await addDecisionHistory(dec)
                }
                setBank((prevBank: Bank | null) =>
                    prevBank ? { ...prevBank, ...reject } : reject,
                )
                setIsOpen(false)
