...
const BankStepBadge: React.FC<BankStepBadgeProps> = ({
    step,
    renovStep,
    finaldec = false,
    isAgent = false,
    ...
    const nkey: any = finaldec
        ? finaldec.status.replace('finalDecisionStatuses.', '')
        : false
    ...
    return (
        <>
                        ...
                        <div>
                            ...
                            {finaldec.createdAt && (
                                <strong className="text-green-400">
                                    {formatRelative(
                                        finaldec?.createdAt.toDate?.() ||
                                            finaldec.createdAt.createdAt,
                                        new Date(),
                                        { locale: fr },
                                    )}
                                </strong>
            ...
            {step == ('bankSteps.needRenovation' as BankStep) && (
                <div
                    className={`inline-flex items-center rounded-md px-2 py-1 text-xs font-medium ring-1 ring-inset  mb-4 bg-indigo-50 text-indigo-600 ring-purple-500/10`}
                >
                    {t(`bank.${renovStep}`)}
                </div>
            )}
